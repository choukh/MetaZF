...
### 工作环境
我们**不需要**第1章的排中律和证明无关性. 我们将会看到, HF结构是可判定的(decidable). 另外, 为了构造模型, 需要导入[Setoid库](https://coq.inria.fr/library/Coq.Setoids.Setoid.html).
...
- 强归纳原理: P 空 → (∀ x y, P x → P y → P (x ⨮ y)) → ∀ x, P x
  ...
  - 强归纳的"强"是说, P不限于到Prop的函数, 而且可以是到Type的函数. 该特性允许我们从存在性中提取对象, 而不需要描述算子(indefinite description). 具体我们将在后面看到.
```Coq
...
### 基本事实
现在, 给定HF的一个模型M, 我们来看一些简单事实, 熟悉一下HF结构下的推理过程.
```Coq
(* 基本事实 *)
Section Basic.
Context {M : HF}.
...
从下一篇开始, 用如下定义的自动化规则能证明的目标, 我们都说"显然成立".
...
最后, 我们设定其递归调用上限是7次, 7次无法解决目标我们就手动证明, 能解决我们就说"显然成立".
```Coq
